...
Usage:
    python3 tests/spec-workflow/test_skill_integration.py [--dry-run] [--model MODEL]
...
SPEC_WORKFLOW_DIR = Path(__file__).parent.resolve()  # tests/spec-workflow/
TESTS_DIR = SPEC_WORKFLOW_DIR.parent                  # tests/
PROJECT_ROOT = TESTS_DIR.parent                        # project root

# Test configuration
TEST_PROJECT_SLUG = "integration-test-hello-world"
TEST_PROJECT_DIR = SPEC_WORKFLOW_DIR / "fixtures" / "generated" / TEST_PROJECT_SLUG
...
def check_api_key() -> bool:
    ...
    if not os.environ.get('ANTHROPIC_API_KEY'):
        ...
        print(f"  python3 tests/spec-workflow/test_skill_integration.py")
        return False
    return True
...
def run_integration_test(dry_run: bool = False, model: str = "claude-sonnet-4-20250514", quick: bool = False):
    ...
    if all_valid:
        ...
        print(f"    ./tests/spec-workflow/test_deliverable_structure.sh {TEST_PROJECT_SLUG}")
        print(f"    python3 tests/spec-workflow/test_adr_format.py {TEST_PROJECT_SLUG}")
    else:
